...
Both only take effect once the `Route LAN` service is enabled on the `bridge` the container belongs to, as shown below. ![](../../overlay/zerotier/1.png)

> Static NAT configuration (the internal target port is the container port, the IP is the container IP) ![](../../overlay/zerotier/2.png)

> IP rule configuration (note this goes in the `destination IP rules` of the `default flow`, assuming you have not made the container's MAC or IP the ingress of some other flow) ![](../../overlay/zerotier/3.png) You can copy the JSON below and paste it into the rule
>
> ```json
> [
>   {
...
Then create a Flow that uses this container as its egress. ![](../../overlay/zerotier/4.png)

## Configuring the "route" rules

Click the `Destination IP` button on the relevant Flow to configure it. Only Flows with a matching rule take effect. ![](../../overlay/zerotier/5.png)

For instance, my LAN client's MAC address is `00:a0:98:27:41:47` and that client is currently governed by the `Flow 11` rules. So I configure `Destination IP` on `Flow 11` and pick the egress as `Flow 21`, the one created when starting the container. ![](../../overlay/zerotier/6.png)
...
Add your internal subnet (mine is `10.10.10.0/24`) to ZeroTier, with the `via` field set to the container's IP you just looked up `(172.26.161.171)`. ![](../../overlay/zerotier/7.png)
...
1. Ping `Device 3` from `Device 1`, handled through `Device 2`. ![](../../overlay/zerotier/8.png)
2. Ping `Device 1` from `Device 3`, handled through `Device 2`. ![](../../overlay/zerotier/9.png)
