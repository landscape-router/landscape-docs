...
然后创建一个 Flow 并使用这个容器作为出口. ![](./dn42/1.png)
...
> 静态NAT配置 (内网目标端口为容器对应wg的端口, IP 为容器 IP) ![](./dn42/3.png)

> 在防火墙放开相应端口 ![](./dn42/4.png)

## 配置 "路由" 规则

点击相应 Flow 的 `目标 IP` 按钮进行配置. 只有添加相应规则的 Flow 才会生效. ![](./dn42/5.png)
