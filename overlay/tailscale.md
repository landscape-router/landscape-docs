...
以上两种方式都只在容器所属的 `网桥` 开启 `Lan 路由转发服务` 时才生效. 如下图. ![](./1.png)

> 静态NAT配置 (内网目标端口为容器端口, IP 为容器 IP) ![](./2.png)
...
然后创建一个 Flow 并使用这个容器作为出口. ![](./3.png)

注意，需要在 tailscale 的控制面板中允许此节点的路由。 ![](./edit-route.png)

![](./allow-route.png) 并且在其他客户端启动时，需要增加 `--accept-routes` 选项, 例如:
...
点击相应 Flow 的 `目标 IP` 按钮进行配置. 只有添加相应规则的 Flow 才会生效. ![](./4.png)

比如我当前 LAN 客户端的 MAC 地址是 `00:a0:98:27:41:47`, 这个客户端当前被 `Flow 11` 规则所管理. 所以我需要在 `Flow 11` 的 `目标 IP` 进行配置. 并选择流量的出口为刚刚启动容器时创建的 `Flow 20`. ![](./5.png)
...
1. 从 `非` 路由部署的 `tailscale 客户端` (100.118.21.86) ping `00:a0:98:27:41:47` 客户端 通过容器中 `100.76.59.45` 处理. ![](./6.png)
2. 从 `00:a0:98:27:41:47` 客户端, ping `非` 路由部署的 `tailscale 客户端` (100.118.21.86) ![](./7.png)
