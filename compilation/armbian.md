...
EOF
	...
	if [ "$BOARD" = "uefi-x86" ]; then
		...
		curl -L -o /root/landscape-webserver https://github.com/ThisSeanZhang/landscape/releases/download/v0.4.8-beta/landscape-webserver-x86_64
		sudo sed -i 's/^GRUB_CMDLINE_LINUX="/GRUB_CMDLINE_LINUX="net.ifnames=0 biosdevname=0 /' /etc/default/grub
		sudo update-grub
	else
		curl -L -o /root/landscape-webserver https://github.com/ThisSeanZhang/landscape/releases/download/v0.4.8-beta/landscape-webserver-aarch64
		# 当 BOARD 为其他值时执行的操作
	...
	curl -L -o /root/static.zip https://github.com/ThisSeanZhang/landscape/releases/download/v0.4.8-beta/static.zip
