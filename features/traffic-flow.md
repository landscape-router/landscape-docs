...
> **示例**：在 [PPPoE 配置](../network/ipv4.md#pppoe)中启用"设置默认路由"开关
...
::: warning 优先级冲突当 DNS 规则和 IP 规则的优先级值相同时，优先使用 DNS 规则。:::
...
::: info 灵活配置并不是该入口的所有流量都从默认出口发出，可以通过域名或 IP 规则指定特定流量使用其他出口。:::
...
::: info 独立缓存每个流都有独立的 DNS 缓存，不用担心同域名在不同流中的缓存冲突。:::
...
::: warning 必须配置每个 Flow 应当至少有一条兜底 DNS 规则，用于处理没有匹配任何规则的域名。:::

兜底规则示例：

![兜底 DNS 规则](./flow-11.png)
...
::: danger 重要提示只有搭配 [**接应程序**](https://github.com/ThisSeanZhang/landscape/blob/main/landscape-ebpf/src/bin/redirect_pkg_handler.rs) 打包的容器，才能作为有效的流出口容器！:::
...
::: tip 提示 [测试接应程序镜像](https://github.com/ThisSeanZhang/landscape/pkgs/container/landscape-edge)已包含接应程序，无需额外添加或挂载。:::
