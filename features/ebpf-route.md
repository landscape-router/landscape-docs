...
::: tip 配置位置在网卡配置界面中，找到对应的 WAN 和 LAN 网卡，开启"路由转发服务"选项。:::
...
::: tip 核心优势 Landscape Router 的转发工作在 **Ingress/Egress (qdisc)** 层完成，即在进入 Netfilter **之前**就决定转发目标并直接发送到网卡。:::
...
::: warning 当前限制由于 NAT 连接信息尚未完全集成到 eBPF 路由中，当前加速效果还不是最优。这是从 0 到 1 的实现，后续会持续优化。:::
